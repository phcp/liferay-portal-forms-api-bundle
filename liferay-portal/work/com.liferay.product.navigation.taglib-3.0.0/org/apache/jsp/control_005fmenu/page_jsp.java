...
ProductNavigationControlMenuEntryRegistry productNavigationControlMenuEntryRegistry = ServletContextUtil.getProductNavigationControlMenuEntryRegistry();
      ...
      out.write('\n');
if (
 !productNavigationControlMenuCategories.isEmpty() ) {
      out.write("\n");
      out.write("\t<div class=\"control-menu control-menu-level-1 hidden-print\" data-qa-id=\"controlMenu\" id=\"");
  ...
  private boolean _jspx_meth_aui_script_0(PageContext _jspx_page_context)
    ...
    if (_jspx_eval_aui_script_0 != javax.servlet.jsp.tagext.Tag.SKIP_BODY) {
      ...
      do {
        ...
        out.write("controlMenu');\n");
        out.write("\n");
        out.write("\t\tvar panelEntryBodies = $('#");
        if (_jspx_meth_portlet_namespace_4((javax.servlet.jsp.tagext.JspTag) _jspx_th_aui_script_0, _jspx_page_context))
          return true;
        out.write("ControlMenu [data-toggle=\"sidenav\"]').toArray().map(\n");
        out.write("\t\t\tfunction(item) {\n");
        out.write("\t\t\t\treturn $(item.getAttribute('data-target').split(',')[0]);\n");
        out.write("\t\t\t}\n");
        out.write("\t\t);\n");
        out.write("\n");
        out.write("\t\tpanelEntryBodies.forEach(\n");
        out.write("\t\t\tfunction(item) {\n");
        out.write("\t\t\t\titem.on(\n");
        out.write("\t\t\t\t\t'openStart.lexicon.sidenav',\n");
        out.write("\t\t\t\t\tfunction(event) {\n");
        out.write("\t\t\t\t\t\tvar itemId = event.target.getAttribute('id');\n");
        out.write("\n");
        out.write("\t\t\t\t\t\tpanelEntryBodies.forEach(\n");
        out.write("\t\t\t\t\t\t\tfunction(item) {\n");
        out.write("\t\t\t\t\t\t\t\tvar panelId = item.attr('id');\n");
        out.write("\n");
        out.write("\t\t\t\t\t\t\t\tif (panelId !== itemId) {\n");
        out.write("\t\t\t\t\t\t\t\t\t$('#");
